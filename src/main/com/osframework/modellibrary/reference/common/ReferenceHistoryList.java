...
null
*/
public class ReferenceHistoryList extends ResultList implements  Enumeration, java.io.Serializable {

	private static final long serialVersionUID = 1L;
	private Vector list = new Vector();
	private ResultColumnList columns = ReferenceHistoryModel.getResultColumnList();
	private transient Enumeration enumeration = list.elements();
	
	public ReferenceHistoryList() {
	
	} 

	public ReferenceHistoryList(int initialCapacity) {
	...
	public void add(ReferenceHistoryModel m) {
		this.list.add(m);
		
	}
	public void addAll(ReferenceHistoryList list) {
	...
	public ReferenceHistoryModel elementAt(int rowIndex) {
			return (ReferenceHistoryModel) list.get(rowIndex);
	}

	public void remove(ReferenceHistoryModel m) {
			list.remove(m);
	}

	public Object getValueAt(int rowIndex, int columnIndex) {
		try {
			ReferenceHistoryModel m = (ReferenceHistoryModel) list.get(rowIndex);
	...
	public ReferenceHistoryModel getFirst() {
		if (list.size() > 0) {
			return (ReferenceHistoryModel) list.elementAt(0);
		} else {
			return new ReferenceHistoryModel();
		}
	}
	...
	public String toString() {
		...
		sb.append("ReferenceHistoryList");
		sb.append("\r\n");
		Enumeration e1 = list.elements();
		while (e1.hasMoreElements()) {
			ReferenceHistoryModel m = (ReferenceHistoryModel) e1.nextElement();
