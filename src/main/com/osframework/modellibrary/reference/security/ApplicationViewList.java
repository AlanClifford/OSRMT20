...
null
*/
public class ApplicationViewList extends ResultList implements  Enumeration, java.io.Serializable {

	private static final long serialVersionUID = 1L;
	private Vector list = new Vector();
	private ResultColumnList columns = ApplicationViewModel.getResultColumnList();
	...
	public ApplicationViewList(int initialCapacity) {
	...
	public void add(ApplicationViewModel m) {
		this.list.add(m);
		
	}
	public void addAll(ApplicationViewList list) {
	...
	public ApplicationViewModel elementAt(int rowIndex) {
			return (ApplicationViewModel) list.get(rowIndex);
	}

	public void remove(ApplicationViewModel m) {
			list.remove(m);
	}

	public Object getValueAt(int rowIndex, int columnIndex) {
		try {
			ApplicationViewModel m = (ApplicationViewModel) list.get(rowIndex);
	...
	public ApplicationViewModel getFirst() {
		if (list.size() > 0) {
			return (ApplicationViewModel) list.elementAt(0);
		} else {
