...
null
*/
public class AppControlTemplateList extends ResultList implements  Enumeration, java.io.Serializable {

	private static final long serialVersionUID = 1L;
	private Vector list = new Vector();
	private ResultColumnList columns = AppControlTemplateModel.getResultColumnList();
	private transient Enumeration enumeration = list.elements();
	
	public AppControlTemplateList() {
	
	} 

	public AppControlTemplateList(int initialCapacity) {
	...
	public void add(AppControlTemplateModel m) {
		this.list.add(m);
		
	}
	public void addAll(AppControlTemplateList list) {
	...
	public AppControlTemplateModel elementAt(int rowIndex) {
			return (AppControlTemplateModel) list.get(rowIndex);
	}

	public void remove(AppControlTemplateModel m) {
			list.remove(m);
	}

	public Object getValueAt(int rowIndex, int columnIndex) {
		try {
			AppControlTemplateModel m = (AppControlTemplateModel) list.get(rowIndex);
	...
	public AppControlTemplateModel getFirst() {
		if (list.size() > 0) {
			return (AppControlTemplateModel) list.elementAt(0);
		} else {
			return new AppControlTemplateModel();
		}
	}
	...
	public String toString() {
		...
		sb.append("AppControlTemplateList");
		sb.append("\r\n");
		Enumeration e1 = list.elements();
		while (e1.hasMoreElements()) {
			AppControlTemplateModel m = (AppControlTemplateModel) e1.nextElement();
