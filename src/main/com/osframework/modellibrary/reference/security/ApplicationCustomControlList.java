...
null
*/
public class ApplicationCustomControlList extends ResultList implements  Enumeration, java.io.Serializable {

	private static final long serialVersionUID = 1L;
	private Vector list = new Vector();
	private ResultColumnList columns = ApplicationCustomControlModel.getResultColumnList();
	private transient Enumeration enumeration = list.elements();
	
	public ApplicationCustomControlList() {
	
	} 

	public ApplicationCustomControlList(int initialCapacity) {
	...
	public void add(ApplicationCustomControlModel m) {
		this.list.add(m);
		
	}
	public void addAll(ApplicationCustomControlList list) {
	...
	public ApplicationCustomControlModel elementAt(int rowIndex) {
			return (ApplicationCustomControlModel) list.get(rowIndex);
	}

	public void remove(ApplicationCustomControlModel m) {
			list.remove(m);
	}

	public Object getValueAt(int rowIndex, int columnIndex) {
		try {
			ApplicationCustomControlModel m = (ApplicationCustomControlModel) list.get(rowIndex);
	...
	public ApplicationCustomControlModel getFirst() {
		if (list.size() > 0) {
			return (ApplicationCustomControlModel) list.elementAt(0);
		} else {
	...
	public String toString() {
		...
		sb.append("ApplicationCustomControlList");
		sb.append("\r\n");
		Enumeration e1 = list.elements();
		while (e1.hasMoreElements()) {
			ApplicationCustomControlModel m = (ApplicationCustomControlModel) e1.nextElement();
