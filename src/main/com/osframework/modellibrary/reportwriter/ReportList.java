...
null
*/
public class ReportList extends ResultList implements  Enumeration, java.io.Serializable {

	private static final long serialVersionUID = 1L;
	private Vector list = new Vector();
	private ResultColumnList columns = ReportModel.getResultColumnList();
	...
	public void add(ReportModel m) {
		this.list.add(m);
		
	}
	public void addAll(ReportList list) {
	...
	public ReportModel elementAt(int rowIndex) {
			return (ReportModel) list.get(rowIndex);
	}

	public void remove(ReportModel m) {
			list.remove(m);
	}

	public Object getValueAt(int rowIndex, int columnIndex) {
		try {
			ReportModel m = (ReportModel) list.get(rowIndex);
	...
	public ReportModel getFirst() {
		if (list.size() > 0) {
			return (ReportModel) list.elementAt(0);
		} else {
