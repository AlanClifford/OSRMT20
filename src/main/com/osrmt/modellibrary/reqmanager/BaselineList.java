...
null
*/
public class BaselineList extends ResultList implements  Enumeration, java.io.Serializable {

	private static final long serialVersionUID = 1L;
	private Vector list = new Vector();
	private ResultColumnList columns = BaselineModel.getResultColumnList();
	...
	public BaselineList(int initialCapacity) {
	...
	public void add(BaselineModel m) {
		this.list.add(m);
		
	}
	public void addAll(BaselineList list) {
	...
	public BaselineModel elementAt(int rowIndex) {
			return (BaselineModel) list.get(rowIndex);
	}

	public void remove(BaselineModel m) {
			list.remove(m);
	}

	public Object getValueAt(int rowIndex, int columnIndex) {
		try {
			BaselineModel m = (BaselineModel) list.get(rowIndex);
	...
	public BaselineModel getFirst() {
		if (list.size() > 0) {
			return (BaselineModel) list.elementAt(0);
		} else {
