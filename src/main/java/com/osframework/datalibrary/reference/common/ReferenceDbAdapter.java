 ...
 * the models depends on the parameters defined in osrmt_schem.mdb ModelLibrary.
*/
public class ReferenceDbAdapter extends BaseAdapter {
	

	private IReferenceMap reference;
	private ISecurity security;

	public ReferenceDbAdapter(IReferenceMap reference, ISecurity security) {
		this.reference = reference;
		this.security = security;
	}


	public UpdateResult UpdateReference(ReferenceModel m, ServiceCall call) throws DataUpdateException {
		DbConnection conn = null;
		try {
			conn = Db.getConnection();
			return UpdateReference(m, call, conn);
		} catch (Exception ex) {
	...
	public UpdateResult UpdateReference(ReferenceModel m, ServiceCall call, DbConnection conn) throws DataUpdateException {
		ReferenceModel original = null;
		try {
			if (m == null) {
			...
			if (m.hasModified()) {
				original = getReference(m.getRefId(), conn);
	...
	private UpdateResult save(ReferenceModel m, DbConnection conn) throws SQLException, AccessDataTypeException {
		int nbrRows = 0;
		String sql = "";
		...
		try {
			if (m.getRefId()==0) {
				m.setRefId(Db.getNextSequence(TableNameFramework.REFERENCE, conn));
				sql = getInsertSql();
			...
			return new UpdateResult(nbrRows,m.getRefId());
	...
	protected int ImportReference(ReferenceModel m) throws SQLException, AccessDataTypeException {
		int nbrRows = 0;
		String sql = "";
		SQLResult result = null;
		Vector params = null;
	...
	public int DeleteReference(ReferenceModel m, DbConnection conn) throws DataUpdateException {
		SQLResult result = null;
		try {
			int nbrRows = 0;
	 ...
	 */
	public void mapReference(ResultSet rset, ReferenceModel m) throws SQLException {
  ...
  */
	public ReferenceList  getReference(String sql) throws DataAccessException {
		DbConnection conn = null;
		try {
			conn = Db.getConnection();
			return getReference(sql, conn);
		} catch (Exception ex) {
  ...
  * Execute the SQL and return a list for the result set
  */
	public ReferenceList  getReference(String sql, DbConnection conn) throws DataAccessException {
		SQLResult result = null;
		try {
  ...
  * Execute the SQL and return a list for the result set
  */
	public ReferenceList  getReference(String sql, Vector params) throws DataAccessException {
		DbConnection conn = null;
		try {
			conn = Db.getConnection();
		return getReference(sql, params, conn);
		} catch (Exception ex) {
			throw new DataAccessException(ex);
		} finally {
			if (conn != null) {
  ...
  * Execute the SQL and return a list for the result set
  */
	public ReferenceList  getReference(String sql, Vector params, DbConnection conn) throws DataAccessException {
		SQLResult result = null;
		try {
			ReferenceList list = new ReferenceList();
	 ...
	 */
	public ReferenceModel getReference(int id) throws DataAccessException { 
		DbConnection conn = null;
		try {
			conn = Db.getConnection();
			return getReference(id, conn);
		} catch (Exception ex) {
			throw new DataAccessException(ex);
		} finally {
			if (conn != null) {
	 ...
	 * Returns an empty model if the id is 0 or not found
	 */
	public ReferenceModel getReference(int id, DbConnection conn) throws DataAccessException { 
		SQLResult result = null;
		try {
			ReferenceModel m = new ReferenceModel();
			...
			Debug.LogException(this, ex, "ReferenceModel" + id);
			throw new DataAccessException(ex);
		} finally {
			try { if (result!=null) result.closeStatements();} catch (Exception ex) {}
		}
	...
	public ReferenceList exportReference() throws DataAccessException {
		int rows = Db.countTable("reference");
		Debug.LogInfo(this, rows + " found in reference");
		String sql = "select * from reference where record_type_ref_id in (?, ?)";
		ReferenceList list = this.getReference(sql, Db.getParameter(RecordTypeFramework.SYSTEMREFERENCE, RecordTypeFramework.USERREFERENCE)); 
		Debug.LogInfo(this,list.size()  + " reference rows exported");
		return list;
	}

	public int importReference(ReferenceList list) throws Exception {
		Enumeration e1 = list.elements();
		int rows = 0;
		while (e1.hasMoreElements()) {
			ReferenceModel m = (ReferenceModel) e1.nextElement();
			rows += ImportReference(m);
		}
	...
	private String getSelectSql() {
		return "select ref_id, reference_group, app_type_ref_id, display_code, "
	...
	private String getInsertSql() {
		return "insert into reference (ref_id, reference_group, app_type_ref_id, display_code, "
			+ "display_sequence, display, description, create_dt, "
			+ "create_user_id, update_dt, update_user_id, update_count, "
			+ "active_ind, system_assigned_version_nbr, record_type_ref_id)"
			+ " values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";				 
	...
	private Vector getInsertParameters(ReferenceModel m) {
	...
	private String getUpdateSql() {
		return "update reference set  reference_group = ?, app_type_ref_id = ?, display_code = ?, "
			+ "display_sequence = ?, display = ?, description = ?, create_dt = ?, "
			+ "create_user_id = ?, update_dt = ?, update_user_id = ?, update_count = ?, "
			+ "active_ind = ?, system_assigned_version_nbr = ?, record_type_ref_id = ? "
			+ " where ref_id = ?";				 
	...
	private Vector getUpdateParameters(ReferenceModel m) {
		Vector v = new Vector(15);
		if (m.getReferenceGroup() != null) v.add(m.getReferenceGroup());
		else v.add(new Null(new String()));
		v.add(new Integer(m.getAppTypeRefId()));
	...
	private Vector getDeleteParameters(ReferenceModel m) {
		Vector v = new Vector(15);
		v.add(new Integer(m.getRefId()));
		return v;
	}
	...
	private String getDeleteSql() {
		return "delete from reference where ref_id = ?";				 
	}
