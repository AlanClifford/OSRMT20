...
null
*/
public class SequenceKeyList extends ResultList implements  Enumeration, java.io.Serializable {

	private static final long serialVersionUID = 1L;
	private Vector list = new Vector();
	private ResultColumnList columns = SequenceKeyModel.getResultColumnList();
	private transient Enumeration enumeration = list.elements();
	
	public SequenceKeyList() {
	
	} 

	public SequenceKeyList(int initialCapacity) {
	...
	public void add(SequenceKeyModel m) {
		this.list.add(m);
	...
	public SequenceKeyModel elementAt(int rowIndex) {
			return (SequenceKeyModel) list.get(rowIndex);
	}

	public void remove(SequenceKeyModel m) {
			list.remove(m);
	}

	public Object getValueAt(int rowIndex, int columnIndex) {
		try {
			SequenceKeyModel m = (SequenceKeyModel) list.get(rowIndex);
	...
	public SequenceKeyModel getFirst() {
		if (list.size() > 0) {
			return (SequenceKeyModel) list.elementAt(0);
		} else {
			return new SequenceKeyModel();
		}
	}
	...
	public void setReferenceDisplay(IReferenceMap reference, ISecurity security) {
		...
		while (e1.hasMoreElements()) {
			SequenceKeyModel m = (SequenceKeyModel) e1.nextElement();
