...
null
*/
public class RecordFileHistoryList extends ResultList implements  Enumeration, java.io.Serializable {

	private static final long serialVersionUID = 1L;
	private Vector list = new Vector();
	private ResultColumnList columns = RecordFileHistoryModel.getResultColumnList();
	private transient Enumeration enumeration = list.elements();
	
	public RecordFileHistoryList() {
	
	} 

	public RecordFileHistoryList(int initialCapacity) {
	...
	public void add(RecordFileHistoryModel m) {
		this.list.add(m);
		
	}
	public void addAll(RecordFileHistoryList list) {
	...
	public RecordFileHistoryModel elementAt(int rowIndex) {
			return (RecordFileHistoryModel) list.get(rowIndex);
	}

	public void remove(RecordFileHistoryModel m) {
			list.remove(m);
	}

	public Object getValueAt(int rowIndex, int columnIndex) {
		try {
			RecordFileHistoryModel m = (RecordFileHistoryModel) list.get(rowIndex);
	...
	public RecordFileHistoryModel getFirst() {
		if (list.size() > 0) {
			return (RecordFileHistoryModel) list.elementAt(0);
		} else {
			return new RecordFileHistoryModel();
		}
	}
	...
	public String toString() {
		...
		sb.append("RecordFileHistoryList");
		sb.append("\r\n");
		Enumeration e1 = list.elements();
		while (e1.hasMoreElements()) {
			RecordFileHistoryModel m = (RecordFileHistoryModel) e1.nextElement();
