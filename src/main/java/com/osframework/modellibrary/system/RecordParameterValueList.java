...
null
*/
public class RecordParameterValueList extends ResultList implements  Enumeration, java.io.Serializable {

	private static final long serialVersionUID = 1L;
	private Vector list = new Vector();
	private ResultColumnList columns = RecordParameterValueModel.getResultColumnList();
	private transient Enumeration enumeration = list.elements();
	
	public RecordParameterValueList() {
	
	} 

	public RecordParameterValueList(int initialCapacity) {
	...
	public void add(RecordParameterValueModel m) {
		this.list.add(m);
		
	}
	public void addAll(RecordParameterValueList list) {
	...
	public RecordParameterValueModel elementAt(int rowIndex) {
			return (RecordParameterValueModel) list.get(rowIndex);
	}

	public void remove(RecordParameterValueModel m) {
			list.remove(m);
	}

	public Object getValueAt(int rowIndex, int columnIndex) {
		try {
			RecordParameterValueModel m = (RecordParameterValueModel) list.get(rowIndex);
	...
	public RecordParameterValueModel getFirst() {
		if (list.size() > 0) {
			return (RecordParameterValueModel) list.elementAt(0);
		} else {
			return new RecordParameterValueModel();
		}
	}
	...
	public String toString() {
		...
		sb.append("RecordParameterValueList");
		sb.append("\r\n");
		Enumeration e1 = list.elements();
		while (e1.hasMoreElements()) {
			RecordParameterValueModel m = (RecordParameterValueModel) e1.nextElement();
