...
null
*/
public class RecordParameterList extends ResultList implements  Enumeration, java.io.Serializable {

	private static final long serialVersionUID = 1L;
	private Vector list = new Vector();
	private ResultColumnList columns = RecordParameterModel.getResultColumnList();
	private transient Enumeration enumeration = list.elements();
	
	public RecordParameterList() {
	
	} 

	public RecordParameterList(int initialCapacity) {
	...
	public void add(RecordParameterModel m) {
		this.list.add(m);
		
	}
	public void addAll(RecordParameterList list) {
	...
	public RecordParameterModel elementAt(int rowIndex) {
			return (RecordParameterModel) list.get(rowIndex);
	}

	public void remove(RecordParameterModel m) {
			list.remove(m);
	}

	public Object getValueAt(int rowIndex, int columnIndex) {
		try {
			RecordParameterModel m = (RecordParameterModel) list.get(rowIndex);
	...
	public RecordParameterModel getFirst() {
		if (list.size() > 0) {
			return (RecordParameterModel) list.elementAt(0);
		} else {
			return new RecordParameterModel();
		}
	}
