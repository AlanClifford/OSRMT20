...
null
*/
public class ReferenceTreeList extends ResultList implements  Enumeration, java.io.Serializable {

	private static final long serialVersionUID = 1L;
	private Vector list = new Vector();
	private ResultColumnList columns = ReferenceTreeModel.getResultColumnList();
	private transient Enumeration enumeration = list.elements();
	
	public ReferenceTreeList() {
	
	} 

	public ReferenceTreeList(int initialCapacity) {
	...
	public void add(ReferenceTreeModel m) {
		this.list.add(m);
		
	}
	public void addAll(ReferenceTreeList list) {
	...
	public ReferenceTreeModel elementAt(int rowIndex) {
			return (ReferenceTreeModel) list.get(rowIndex);
	}

	public void remove(ReferenceTreeModel m) {
			list.remove(m);
	}

	public Object getValueAt(int rowIndex, int columnIndex) {
		try {
			ReferenceTreeModel m = (ReferenceTreeModel) list.get(rowIndex);
	...
	public ReferenceTreeModel getFirst() {
		if (list.size() > 0) {
			return (ReferenceTreeModel) list.elementAt(0);
		} else {
