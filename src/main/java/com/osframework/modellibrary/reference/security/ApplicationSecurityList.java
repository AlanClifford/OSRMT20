...
null
*/
public class ApplicationSecurityList extends ResultList implements  Enumeration, java.io.Serializable {

	private static final long serialVersionUID = 1L;
	private Vector list = new Vector();
	private ResultColumnList columns = ApplicationSecurityModel.getResultColumnList();
	private transient Enumeration enumeration = list.elements();
	
	public ApplicationSecurityList() {
	
	} 

	public ApplicationSecurityList(int initialCapacity) {
	...
	public void add(ApplicationSecurityModel m) {
		this.list.add(m);
		
	}
	public void addAll(ApplicationSecurityList list) {
	...
	public ApplicationSecurityModel elementAt(int rowIndex) {
			return (ApplicationSecurityModel) list.get(rowIndex);
	}

	public void remove(ApplicationSecurityModel m) {
			list.remove(m);
	}

	public Object getValueAt(int rowIndex, int columnIndex) {
		try {
			ApplicationSecurityModel m = (ApplicationSecurityModel) list.get(rowIndex);
	...
	public ApplicationSecurityModel getFirst() {
		if (list.size() > 0) {
			return (ApplicationSecurityModel) list.elementAt(0);
		} else {
			return new ApplicationSecurityModel();
		}
	}
	...
	public String toString() {
		...
		sb.append("ApplicationSecurityList");
		sb.append("\r\n");
		Enumeration e1 = list.elements();
		while (e1.hasMoreElements()) {
			ApplicationSecurityModel m = (ApplicationSecurityModel) e1.nextElement();
