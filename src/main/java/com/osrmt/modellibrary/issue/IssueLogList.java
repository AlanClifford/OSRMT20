...
null
*/
public class IssueLogList extends ResultList implements  Enumeration, java.io.Serializable {

	private static final long serialVersionUID = 1L;
	private Vector list = new Vector();
	private ResultColumnList columns = IssueLogModel.getResultColumnList();
	...
	public IssueLogList(int initialCapacity) {
	...
	public void add(IssueLogModel m) {
		this.list.add(m);
		
	}
	public void addAll(IssueLogList list) {
	...
	public IssueLogModel elementAt(int rowIndex) {
			return (IssueLogModel) list.get(rowIndex);
	}

	public void remove(IssueLogModel m) {
			list.remove(m);
	}

	public Object getValueAt(int rowIndex, int columnIndex) {
		try {
			IssueLogModel m = (IssueLogModel) list.get(rowIndex);
	...
	public IssueLogModel getFirst() {
		if (list.size() > 0) {
			return (IssueLogModel) list.elementAt(0);
		} else {
			return new IssueLogModel();
		}
	}
