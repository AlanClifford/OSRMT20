...
null
*/
public class RequirementTreeHistoryList extends ResultList implements  Enumeration, java.io.Serializable {

	private static final long serialVersionUID = 1L;
	private Vector list = new Vector();
	private ResultColumnList columns = RequirementTreeHistoryModel.getResultColumnList();
	private transient Enumeration enumeration = list.elements();
	
	public RequirementTreeHistoryList() {
	
	} 

	public RequirementTreeHistoryList(int initialCapacity) {
	...
	public void add(RequirementTreeHistoryModel m) {
		this.list.add(m);
		
	}
	public void addAll(RequirementTreeHistoryList list) {
	...
	public RequirementTreeHistoryModel elementAt(int rowIndex) {
			return (RequirementTreeHistoryModel) list.get(rowIndex);
	}

	public void remove(RequirementTreeHistoryModel m) {
			list.remove(m);
	}

	public Object getValueAt(int rowIndex, int columnIndex) {
		try {
			RequirementTreeHistoryModel m = (RequirementTreeHistoryModel) list.get(rowIndex);
	...
	public RequirementTreeHistoryModel getFirst() {
		if (list.size() > 0) {
			return (RequirementTreeHistoryModel) list.elementAt(0);
		} else {
			return new RequirementTreeHistoryModel();
		}
	}
	...
	public String toString() {
		...
		sb.append("RequirementTreeHistoryList");
		sb.append("\r\n");
		Enumeration e1 = list.elements();
		while (e1.hasMoreElements()) {
			RequirementTreeHistoryModel m = (RequirementTreeHistoryModel) e1.nextElement();
