...
Artfiact History stores the image of a artifact prior to modification - along with the user/date who modified the record.  Additionally all baselines are stored.
*/
public class ArtifactHistoryList extends ResultList implements  Enumeration, java.io.Serializable {

	private static final long serialVersionUID = 1L;
	private Vector list = new Vector();
	private ResultColumnList columns = ArtifactHistoryModel.getResultColumnList();
	...
	public ArtifactHistoryList(int initialCapacity) {
	...
	public void add(ArtifactHistoryModel m) {
		this.list.add(m);
		
	}
	public void addAll(ArtifactHistoryList list) {
	...
	public ArtifactHistoryModel elementAt(int rowIndex) {
			return (ArtifactHistoryModel) list.get(rowIndex);
	}

	public void remove(ArtifactHistoryModel m) {
			list.remove(m);
	}

	public Object getValueAt(int rowIndex, int columnIndex) {
		try {
			ArtifactHistoryModel m = (ArtifactHistoryModel) list.get(rowIndex);
	...
	public ArtifactHistoryModel getFirst() {
		if (list.size() > 0) {
			return (ArtifactHistoryModel) list.elementAt(0);
		} else {
			return new ArtifactHistoryModel();
		}
	}
