...
null
*/
public class ArtifactDocumentList extends ResultList implements  Enumeration, java.io.Serializable {

	private static final long serialVersionUID = 1L;
	private Vector list = new Vector();
	private ResultColumnList columns = ArtifactDocumentModel.getResultColumnList();
	private transient Enumeration enumeration = list.elements();
	
	public ArtifactDocumentList() {
	
	} 

	public ArtifactDocumentList(int initialCapacity) {
	...
	public void add(ArtifactDocumentModel m) {
		this.list.add(m);
		
	}
	public void addAll(ArtifactDocumentList list) {
	...
	public ArtifactDocumentModel elementAt(int rowIndex) {
			return (ArtifactDocumentModel) list.get(rowIndex);
	}

	public void remove(ArtifactDocumentModel m) {
			list.remove(m);
	}

	public Object getValueAt(int rowIndex, int columnIndex) {
		try {
			ArtifactDocumentModel m = (ArtifactDocumentModel) list.get(rowIndex);
	...
	public ArtifactDocumentModel getFirst() {
		if (list.size() > 0) {
			return (ArtifactDocumentModel) list.elementAt(0);
		} else {
			return new ArtifactDocumentModel();
		}
	}
	...
	public String toString() {
		...
		sb.append("ArtifactDocumentList");
		sb.append("\r\n");
		Enumeration e1 = list.elements();
		while (e1.hasMoreElements()) {
			ArtifactDocumentModel m = (ArtifactDocumentModel) e1.nextElement();
